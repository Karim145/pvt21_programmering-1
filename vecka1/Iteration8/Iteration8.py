from vecka1.Iteration8 import api_module
...
def check_correct_input(your_answer,antal_svar):
    if your_answer>antal_svar or your_answer<=0:
        ...
        print(f"Frågan har {antal_svar} svar, och endast siffror 1-{antal_svar} accepteras som input. Försök igen! ")
        return "error"
def get_questions():
    res = []
    for q in randomlist:
...
def main():
    ...
    for question in get_questions():
        ...
        while True:
            ...
            antal_svar = i
            try:
                your_answer = input(f"Svara här --> ")
                check_error= check_correct_input(int(your_answer),int(antal_svar))
                ...
                print("Det var ingen siffra!  Var vänlig och försök igen! ")
                print("-" * 80)
